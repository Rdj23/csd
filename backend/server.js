...
app.post("/api/auth/google", async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, user: ticket.getPayload(), token: "mock-jwt-token" });
  } catch (error) {
...
app.get("/api/users", async (req, res) => {
  try {
    let allUsers = [], cursor = null, hasMore = true;
    while (hasMore) {
      const response = await axios.get(`${DEVREV_API}/dev-users.list${cursor ? `?cursor=${cursor}` : ""}`, { headers: HEADERS });
      allUsers = [...allUsers, ...(response.data.dev_users || [])];
      cursor = response.data.next_cursor;
      hasMore = !!cursor;
    }
    res.json(allUsers.map(u => ({ id: u.id, display_name: u.display_name, full_name: u.full_name, email: u.email })));
...
const getUserShiftStatus = (userName) => {
  ...
  if (!userRow) {
    const similar = ROSTER_ROWS.map((r) => r[NAME_COL_INDEX]).filter((n) => n && String(n).toLowerCase().startsWith(userName.charAt(0).toLowerCase())).slice(0, 3);
    return { isActive: false, status: "Not in Roster", candidates: similar };
  }

  const istNow = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
  const dateObj = new Date(istNow);
  const todayKey = `${dateObj.getDate()}-${dateObj.toLocaleString("default", { month: "short" })}`;
  const todayColIdx = DATE_COL_MAP[todayKey];

  if (todayColIdx === undefined) return { isActive: false, status: "Date Not Found", shiftName: "Unknown" };

  const shiftName = userRow[todayColIdx]?.trim();
  const cleanShift = Object.keys(SHIFT_TIMINGS).find((k) => shiftName && shiftName.includes(k));

  if (!cleanShift) return { isActive: false, status: shiftName || "Off Duty", shiftName };
  ...
  let isActive = endVal < startVal ? (nowVal >= startVal || nowVal <= endVal) : (nowVal >= startVal && nowVal <= endVal);
  return { isActive, status: isActive ? "Active" : "Away", shiftName: cleanShift, timings: `${start} - ${end}` };
};

const syncRoster = async () => {
  try {
    console.log("🔁 Auto-syncing roster...");
    const auth = new google.auth.GoogleAuth({ keyFile: "google-sheets-key.json", scopes: ["https://www.googleapis.com/auth/spreadsheets.readonly"] });
    ...
    const getRows = await googleSheets.spreadsheets.values.get({ spreadsheetId, range: sheetName, valueRenderOption: "FORMATTED_VALUE" });
    const rawRows = getRows.data.values || [];
    if (!rawRows.length) return;

    let headerRowIndex = 0;
    for (let i = 0; i < Math.min(rawRows.length, 20); i++) {
      if (JSON.stringify(rawRows[i]).includes("Designation")) { headerRowIndex = i; break; }
    }
    ...
    headerRow.forEach((col, index) => {
      if (/^\d{1,2}-[A-Za-z]{3}$/.test(cleanString(col))) DATE_COL_MAP[col.trim()] = index;
    });

    ROSTER_ROWS = rawRows.slice(headerRowIndex + 1).filter((row) => row[NAME_COL_INDEX] && String(row[NAME_COL_INDEX]).trim().length > 2);
...
app.post("/api/profile/status", async (req, res) => {
  ...
  if (!shiftStatus.isActive && ROSTER_ROWS.length > 0) {
    backups = ROSTER_ROWS.filter((row) => {
      ...
      if (teamMembers && teamMembers.length > 0 && !teamMembers.some((member) => isNameMatch(rName, member))) return false;
      return getUserShiftStatus(rName).isActive;
    }).slice(0, 2).map((row) => row[NAME_COL_INDEX]);
  }
  ...
  if (ticketCount === 0) {
    ...
    const criticalTickets = activeTickets.filter((t) => t.stage === "Waiting on Assignee" && (String(t.severity).toLowerCase().includes("high") || String(t.severity).toLowerCase().includes("blocker")));
    const uniqueAccounts = [...new Set(criticalTickets.map((t) => t.account).filter(a => a && a !== "Unknown"))].slice(0, 2);
...
app.post("/api/analytics/insight", async (req, res) => {
  ...
  try {
    ...
    let prompt = comparison 
      ? `Compare ${context}'s ${metric} vs Team. User: ${comparison.userVal}, Team: ${comparison.teamVal}. Trend: ${JSON.stringify(chartData.slice(-7))}. Better or worse? 1 tip. Max 25 words.`
      : `Analyze ${metric} trend for ${context}. Data: ${JSON.stringify(chartData.slice(-10))}. Trend & Why. Max 20 words.`;
...
app.post("/api/notify/eta", (req, res) => res.json({ success: true, message: "Slack notification sent" }));
