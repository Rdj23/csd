...
**Since 10 Aug 2026 alerts go to each member's TEAM channel**, not one shared channel. The team → channel mapping lives in the backend, on the `TEAMS` array in `backend/config/constants.js` (`slackChannel` per team) — the same array you edit for every roster change, so team reshuffles stay a one-file edit. The backend resolves the channel and sends it in the payload; the n8n Slack node just posts to whatever `channel` says. Members whose team has `slackChannel: ""`, and teamless members (`TEAMLESS_MEMBERS`) with no `slackChannel` of their own, get **no Slack alerts at all** — queues still build for the dashboard, posts are skipped with a log. A teamless member *can* be given a personal `slackChannel` to opt back into alerts; since the 2026-09-08 restructure none are, because everyone teamless has left GST.
