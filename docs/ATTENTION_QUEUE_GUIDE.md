...
- All alerts are delivered through **n8n** (see `docs/ATTENTION_N8N_SETUP.md`). Each alert goes to the member's **team channel**, mapped via `slackChannel` on the `TEAMS` array in `backend/config/constants.js` — team changes are a constants-only edit, n8n never changes. Members with an unset `slackChannel` — an empty one on their team, or none of their own if they are teamless (`TEAMLESS_MEMBERS`) — get **no Slack alerts**; their queues still build for the dashboard. Teamless members may carry a personal `slackChannel` to keep alerts, though as of the 2026-09-08 restructure none do.
